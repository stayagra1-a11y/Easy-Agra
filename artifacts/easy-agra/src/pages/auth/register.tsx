...
import { Map, Eye, EyeOff, Loader2, Upload, User } from "lucide-react";
        ...
        <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6">
          ...
          <div className="flex flex-col items-center mb-5">
            <div className="relative">
              <div className="h-16 w-16 rounded-full border-2 border-primary/30 overflow-hidden bg-muted flex items-center justify-center">
                {photoPreview
                  ? <img src={photoPreview} className="h-full w-full object-cover" alt="Profile" />
                  : <User className="h-8 w-8 text-muted-foreground" />}
              </div>
              <label htmlFor="photo" className="absolute -bottom-1 -right-1 h-6 w-6 rounded-full bg-primary text-white flex items-center justify-center cursor-pointer hover:bg-primary/90">
                <Upload className="h-3 w-3" />
              </label>
              <input id="photo" type="file" accept="image/*" className="hidden" onChange={handlePhotoChange} />
            </div>
            <p className="text-xs text-muted-foreground mt-1">Profile photo (optional)</p>
          </div>
